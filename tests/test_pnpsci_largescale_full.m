function test_pnpsci_largescale_full(dataname)
%TEST_PNPSCI_LARGESCALE_FULL Test Plug-and-Play algorithms for Snapshot 
%Compressive Imaging (PnP-SCI) of large-scale datasets. Here we include the 
%deep denoiser FFDNet as image/video priors along with TV as 
%denoisers/priors for four simulated largescale video-SCI datasets 
%(in Bayer-RGB color).
%   TEST_PNPSCI_LARGESCALE_FULL(dataname) runs the PnP-SCI algorithms with
...
[vgaptv_bayer,psnr_gaptv,ssim_gaptv,tgaptv,psnrall_tv] = ...
...
fprintf('%s-%s mean PSNR %2.2f dB, mean SSIM %.4f, total time % 4.1f s.\n',...
    upper(para.projmeth),upper(para.denoiser),mean(psnr_gaptv),mean(ssim_gaptv),tgaptv);

vgaptv = vdemosaic(vgaptv_bayer,para); % video demosaic

write_video(vgaptv,framerate,sprintf('%s/%s%d_%s-%s.avi',bayervideodir,...
    para.name,nframe*nmask,upper(para.projmeth),upper(para.denoiser)));
...
save([matdir '/pnpsci_largescale_full_' dataname num2str(nframe*nmask) ...
      '.mat'],'-v7.3');
